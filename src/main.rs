...
fn load_and_play(
    ...
    bar_start: Option<&str>,
    pattern_start: Option<&str>,
) {
    ...
    let start_bar: u16 = { bar_start.unwrap_or("1").parse().unwrap_or(0) };

    let play_result = &chord_composer::play_from(
        file_path,
        &mut performance_state,
        play_with_metronome,
        &sample_paths_metronome,
        &sample_paths_piano,start_bar, pattern_start
    );
    parse_result(play_result);
}
    ...
    let matches = App::new(strings::STRING_TITLE)
        ...
        .subcommand(
            SubCommand::with_name("play")
                ...
                .arg(
                    Arg::with_name("pattern-start")
                        .long("pattern-start")
                        .value_name("pattern-start")
                        .help("TODO")
                        .takes_value(true),
                )
                .arg(
                    Arg::with_name("bar-start")
                        .long("bar-start")
                        .value_name("bar-start")
                        .help("TODO")
                        .takes_value(true),
                )
                .arg(
    ...
    if let Some(matches) = matches.subcommand_matches("play") {
        match matches.value_of("COMPOSITION_FILE") {
            Some(file_name) => load_and_play(
                ...
                matches.value_of("bar-start"),
                matches.value_of("pattern-start"),
            ),
            None => println!(
                "{} {}",
